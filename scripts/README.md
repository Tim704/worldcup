...
| File               | Purpose                                                        |
|--------------------|----------------------------------------------------------------|
| `fixtures.py`      | Pure data layer: `Fixture` dataclass, `SAMPLE_FIXTURES`, `normalize()`. No I/O. |
...
If `FIXTURE_FEED_URL` is omitted, the service seeds the table from
`fixtures.SAMPLE_FIXTURES` (eight realistic group-stage matches across US, CA and
MX host venues) — handy for local development without a live feed.

## Fetch + fallback behaviour

`fetch_fixtures(feed_url)`:

1. No `FIXTURE_FEED_URL` → return `SAMPLE_FIXTURES` immediately.
...
3. **Any** failure (network error, non-2xx status, malformed JSON, empty list,
   all-invalid rows) logs a warning and falls back to `SAMPLE_FIXTURES`, so the
   loop never crashes and the table is always seeded.
